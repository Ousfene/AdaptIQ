...
import { generateQuestion, generateHint, submitAnswer } from '../services/apiService';
...
const ClassicRoom: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (step === 'quiz' && !isAnswered && timeLeft > 0) {
      ...
      setIsAnswered(true); // Auto-answer (wrong) when time runs out
    }
 ...
 const handleAnswer = (answer: string) => {
    ...
    // Tell backend — fire and forget (don't block the UI)
    if (currentQuestion) {
      submitAnswer({
  ...
  const handleHint = async () => {
    ...
    try {
      const hintText = await generateHint(currentQuestion.text, currentQuestion.correctAnswer);
      setHint(hintText);
      setSession(prev => prev ? {
        ...prev,
          ...
          <div className="grid grid-cols-1 gap-6 mb-12">
            {[
              ...
              <button
                ...
                disabled={isLoading}
                className="group relative bg-white p-8 border border-[#2D1B14]/10 hover:border-[#D4AF37] transition-all text-left flex items-center gap-8 shadow-sm hover:shadow-xl"
              >
      ...
      <InternalLayout>
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            {!isAnswered && (
              <div className="flex justify-center">
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 border border-[#D4AF37]/30 text-[10px] font-bold uppercase tracking-widest text-[#D4AF37] hover:bg-[#D4AF37] hover:text-[#2D1B14] transition-all disabled:opacity-50"
                >
            ...
            {hint && !isAnswered && (
              <div className="p-6 bg-[#FDFCF7] border-l-4 border-[#D4AF37] italic text-sm text-[#2D1B14]/70 font-serif animate-in fade-in slide-in-from-top-2">
            ...
            {(isAnswered || timeLeft === 0) && (
              <div className="p-8 bg-white border border-[#2D1B14]/10 shadow-lg animate-in fade-in slide-in-from-bottom-4">
