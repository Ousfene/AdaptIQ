...
const cleanTopicDescription = (description: string): string => {
  return (description || '').replace(/â€“/g, '-').trim();
};
...
const CustomRoom: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (screen === 'quiz' && !isAnswered && timeLeft > 0) {
      timer = setInterval(() => setTimeLeft(prev => prev - 1), 1000);
    } else if (timeLeft === 0 && !isAnswered) {
      handleAnswerSubmit('__timeout__'); // timeout = wrong answer
    }
  ...
  // Reset transient question UI state before loading the next prompt.
  ...
  const startQuiz = async (topic: string, conceptId?: string, conceptName?: string) => {
    setIsLoading(true);
    try {
      // 1. Create session on backend, get session_id + initial progress
      const sessionData = await startCustomSession(topic, conceptId);

      // 2. Generate the first question
      const firstQ = await generateCustomQuestion(sessionData.session_id, topic, conceptId);
  ...
  const handleNextQuestion = async () => {
    ...
    resetQuestionState();
    try {
      const nextQ = await generateCustomQuestion(session.session_id, session.topic, session.concept_id);
      setSession(prev => {
        if (!prev) return null;
        return {
          ...prev,
  ...
  const handleGetHint = async () => {
    ...
    try {
      const currentQ = session.questions[session.currentIndex];
      const res = await fetch(`${API_BASE}/api/custom/generate-hint`, {
        method: 'POST',
        headers: authHeaders(),
    ...
    <div className="max-w-4xl mx-auto text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <button
          ...
          className="group relative bg-white p-12 border border-[#2D1B14]/10 hover:border-[#D4AF37] transition-all duration-500 shadow-sm hover:shadow-2xl overflow-hidden"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {!isCatalogueLoading && !catalogueError && historyTopics.map((theme) => (
          <button
            ...
            className="group bg-white p-8 border border-[#2D1B14]/10 hover:border-[#D4AF37] transition-all text-left shadow-sm hover:shadow-xl"
          >
            <div className="w-12 h-12 bg-[#F5F2E7] rounded-sm flex items-center justify-center mb-6 group-hover:bg-[#D4AF37] group-hover:text-[#2D1B14] transition-colors">
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2">
          ...
          <div className="bg-white p-8 border border-[#2D1B14]/10 shadow-inner min-h-[420px]">
            {isCatalogueLoading ? (
                  ...
                  <button
                    ...
                    className={`group p-6 border transition-all text-left ${
                      selectedCountry === country.name
                        ? 'border-[#D4AF37] bg-[#FDFCF7] shadow-lg'
                        : 'border-[#2D1B14]/10 bg-white hover:border-[#D4AF37]/60 hover:shadow-md'
                    }`}
                  >
          ...
          <AnimatePresence mode="wait">
            {selectedGeographyTopic ? (
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className="bg-white p-10 border-2 border-[#D4AF37] shadow-2xl text-center"
              >
                ...
                <button
                  ...
                  className="w-full py-4 bg-[#2D1B14] text-[#F5F2E7] text-[10px] font-bold uppercase tracking-[0.3em] hover:bg-[#3d261c] transition-all shadow-lg"
                >
                  Begin Inquiry
                </button>
              </motion.div>
  ...
  const renderQuiz = () => {
    ...
    const currentQ: CustomQuestion = session.questions[session.currentIndex];
        ...
        <div className="flex justify-between items-center mb-12 pb-6 border-b border-[#D4AF37]/20">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="text-center hidden sm:block">
              ...
              <div className="w-24 h-2 bg-[#2D1B14]/10 rounded-full overflow-hidden">
                <div
            ...
            <div className="text-center">
              ...
              <div className={`text-2xl font-black font-playfair ${timeLeft < 10 ? 'text-red-500 animate-pulse' : 'text-[#2D1B14]'}`}>
                {timeLeft}s
              </div>
            </div>
            <div className="text-center">
                ...
                return (
                  <button
                    ...
                    className={`p-6 text-left border transition-all flex items-center justify-between group ${
                      ...
                      isSelected ? 'border-[#D4AF37] bg-[#FDFCF7]' :
              ...
              <div className="flex flex-col items-center gap-4">
                {hint ? (
                  <motion.div
                    ...
                    className="w-full p-6 bg-[#D4AF37]/5 border border-[#D4AF37]/20 italic text-[#2D1B14]/70 text-center"
                  >
  ...
  const renderSummary = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center">
        ...
        {/* Progress update */}
        <div className="mb-8 p-6 bg-white border border-[#D4AF37]/40">
