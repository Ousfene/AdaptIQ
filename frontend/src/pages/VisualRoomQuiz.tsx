...
const VisualRoomQuiz: React.FC = () => {
  ...
  const handleHint = async () => {
    ...
    try {
      const hintText = await fetchVisualHint(question.id);
      setHint(hintText);
    ...
    return (
      <InternalLayout>
        ...
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 gap-6 mb-10">
            {(
              ...
              <button
                ...
                disabled={isLoading}
                className={`group relative bg-white p-8 border transition-all text-left flex items-center gap-8 shadow-sm hover:shadow-xl ${
                  topic === t.id ? 'border-[#D4AF37]' : 'border-[#2D1B14]/10 hover:border-[#D4AF37]'
                }`}
              >
                <div className={`w-16 h-16 bg-[#F5F2E7] flex items-center justify-center transition-all ${
          ...
          <div className="bg-white p-8 border border-[#2D1B14]/10 shadow-sm mb-10 text-left">
                ...
                <button
                  ...
                  className={`py-3 border text-[10px] font-bold uppercase tracking-widest transition-all ${
                    level === lvl
                      ...
                      : 'bg-white text-[#2D1B14] border-[#2D1B14]/10 hover:border-[#D4AF37]'
                  }`}
                >
                  {lvl}
                </button>
          ...
          <button
            onClick={handleStart}
            disabled={isLoading}
            className="w-full py-4 bg-[#2D1B14] text-[#F5F2E7] text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-[#3d261c] transition-colors flex items-center justify-center gap-3"
          >
      ...
      <InternalLayout>
        <div className="max-w-4xl mx-auto py-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div className="bg-white p-8 border border-[#2D1B14]/10 shadow-sm text-center">
          ...
          <div className="bg-[#2D1B14] p-10 text-[#F5F2E7] mb-12">
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                className="px-8 py-4 bg-[#D4AF37] text-[#2D1B14] font-bold uppercase tracking-widest text-xs hover:bg-[#e6c75a] transition-colors"
              >
                Run Again
              </button>
              <button
                onClick={() => navigate('/dashboard')}
                className="px-8 py-4 border border-white/20 text-white font-bold uppercase tracking-widest text-xs hover:bg-white/10 transition-colors"
              >
                Dashboard
              </button>
            </div>
  ...
  return (
    <InternalLayout>
      <div className="max-w-4xl mx-auto">
        ...
        {question && (
          <>
            <div className="bg-white p-6 border border-[#2D1B14]/10 shadow-sm mb-8">
              <div className="text-[10px] font-bold uppercase tracking-widest opacity-40 mb-3">Observe</div>
              <div className="w-full bg-[#F5F2E7] border border-[#2D1B14]/10 flex items-center justify-center min-h-[200px]">
                {question.show_shape && question.shape_svg ? (
                  <div
                    className="w-full max-h-[360px] flex items-center justify-center p-4"
                    dangerouslySetInnerHTML={{ __html: question.shape_svg }}
                  />
                ) : (
                  <img
            ...
            <div className="bg-white p-8 border border-[#2D1B14]/10 shadow-sm mb-8">
              <div className="flex items-start justify-between gap-6">
                ...
                <button
                  ...
                  className={`px-4 py-3 border text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 transition-colors ${
                    hint ? 'opacity-50 cursor-not-allowed border-[#2D1B14]/10' : 'border-[#D4AF37]/50 hover:bg-[#D4AF37]/10'
                  }`}
                >
              ...
              {hint && (
                <div className="mt-6 p-5 bg-[#F5F2E7] border border-[#2D1B14]/10">
            ...
            <div className="bg-white p-8 border border-[#2D1B14]/10 shadow-sm">
                          ...
                          isCorrect
                            ...
                            : isWrong
                              ...
                              : isSelected
                                ...
                                : 'bg-white border-[#2D1B14]/10 hover:border-[#D4AF37]'
                        }`}
                      >
              ...
              {isTextMode && (
                <div className="space-y-4">
                  <input
                    ...
                    className="w-full border border-[#2D1B14]/20 p-4 text-sm"
                  />
                  <button
                    onClick={() => void handleSubmit(textAnswer)}
                    disabled={isAnswered}
                    className="px-6 py-4 bg-[#2D1B14] text-[#F5F2E7] text-[10px] font-bold uppercase tracking-widest hover:bg-[#3d261c] transition-colors"
                  >
                    Submit
                  </button>
                </div>
              ...
              {feedback && (
                <div className="mt-8 border-t border-[#2D1B14]/10 pt-8">
                  <div className={`p-6 border ${feedback.is_correct ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
