...
import { getUserRank, startChallengeSession, submitChallengeAnswer, generateChallengeQuestion, endChallengeSession } from '../services/challengeService';
...
const ChallengeRoom: React.FC = () => {
  ...
  try {
    ...
    setSession(prev => {
      ...
      return {
        ...
        streak_correct: result.is_correct ? prev.streak_correct + 1 : 0,
        streak_wrong: !result.is_correct ? prev.streak_wrong + 1 : 0,
  ...
  const handleNextQuestion = async () => {
    ...
    try {
      // Try to consume a prefetched question from localStorage cache first
      let nextQ;
      try {
        const key = `challenge_prefetch_${session.session_id}`;
      ...
      if (!nextQ) {
        nextQ = await generateChallengeQuestion(session.topic, session.current_level, session.session_id);
      }
      setSession(prev => {
        if (!prev) return null;
        return {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {([1, 2, 3, 4, 5] as ChallengeLevel[]).map((lvl) => {
              ...
              return (
                ...
                >
                  <div className={`w-12 h-12 mb-6 flex items-center justify-center font-black text-xl border-2 ${
            ...
            {currentQ.is_free_text ? (
              <div className="space-y-4">
                <input
                  ...
                  className={`w-full p-6 bg-white border text-xl font-serif italic outline-none transition-all ${
                    hasReview
                      ? questionReview?.is_correct
                        ? 'border-green-500 bg-green-50 text-green-900'
                        : 'border-red-500 bg-red-50 text-red-900'
                      : 'border-[#2D1B14]/10 focus:border-[#D4AF37]'
                  }`}
                />
                {!isAnswered && (
                  <button
                  ...
                  return (
                    <button
                      ...
                      className={`p-6 text-left border transition-all flex items-center justify-between group ${
                        ...
                        isSelected && !hasReview ? 'border-[#D4AF37] bg-[#FDFCF7]' :
