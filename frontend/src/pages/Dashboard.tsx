...
//   5. User ID is read from adaptiq_user_id (legacy fallback: user_id)
...
const DashboardWrapper: React.FC = () => {
  ...
  // ── On mount: resolve auth user id from localStorage, fetch onboarding status ──
  React.useEffect(() => {
    const storedId = localStorage.getItem('adaptiq_user_id') || localStorage.getItem('user_id');
    if (!storedId) return;  // not logged in yet

    // Normalize legacy key usage so downstream code consistently finds adaptiq_user_id.
    if (!localStorage.getItem('adaptiq_user_id')) {
      localStorage.setItem('adaptiq_user_id', storedId);
    }

    setUserId(storedId);

    getOnboardingStatus(storedId)
      .then(status => {
        setStats(prev => ({
          ...prev,
