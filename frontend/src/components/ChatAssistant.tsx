...
const ChatAssistant: React.FC = () => {
  ...
  // Determine user id for localStorage persistence
  const getUserIdKey = () => {
    const userId = user?.id || localStorage.getItem('adaptiq_user_id') || 'guest';
    return `adaptiq_scholar_history_${userId}`;
  };

  // 2. Hydrate Messages on Mount
  useEffect(() => {
    if (!user || isRoomActive) {
      setMessages([]);
    ...
    const stored = localStorage.getItem(getUserIdKey());
    if (stored) {
      try {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (index >= totalLength) {
        clearInterval(timer);
        setStreamingId(null);
        // Persist only once completed to historical standard
        const fullList = [...messages];
        localStorage.setItem(getUserIdKey(), JSON.stringify(fullList.slice(-30)));
      }
    }, 12);
