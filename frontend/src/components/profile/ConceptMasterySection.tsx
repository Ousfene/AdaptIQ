...
export default function ConceptMasterySection({ concepts }: ConceptMasterySectionProps) {
  ...
  if (concepts.length === 0) {
    return (
      <div className="mb-12 rounded-sm border border-[#2D1B14]/10 bg-white p-8 shadow-sm">
    ...
    <div className="mb-12 rounded-sm border border-[#2D1B14]/10 bg-white p-8 shadow-sm">
      ...
      <div className="space-y-4">
        {concepts.slice(0, 12).map((concept) => {
          ...
          return (
            <div key={concept.concept_id} className="rounded-sm border border-[#2D1B14]/10 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                ...
                <div className="rounded-full bg-[#D4AF37]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#2D1B14]">
