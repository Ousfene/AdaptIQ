...
function getUserId(): string {
  ...
  if (!id) {
    throw new Error('No authenticated user id found in local storage. Please login again.');
  }
  return id;
}
...
export async function generateCustomQuestion(
  ...
  const payload: GenerateQuestionRequest = { session_id, topic, concept_id };
...
export async function endCustomSession(session_id: string): Promise<EndSessionResponse> {
  const res = await fetch(`${BASE_URL}/session/${session_id}/end`, {
    method: 'POST',
    headers: authHeaders(),
  });
...
export async function getConceptsForTopic(topic: string): Promise<ConceptOut[]> {
  const topicFamily = topic.toLowerCase().startsWith('history')
    ...
    : topic.toLowerCase().startsWith('geography')
      ? 'geography'
      : 'mixed';
  const res = await fetch(`${BASE_URL}/concepts/${topicFamily}`, {
    headers: authHeaders(),
  });
...
export async function getUserConceptMastery(userId?: string): Promise<ConceptMasteryResponse> {
  ...
  const res = await fetch(`${BASE_URL}/user/${uid}/concept-mastery`, {
    headers: authHeaders(),
  });
