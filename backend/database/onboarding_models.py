...
Onboarding models â€” user flags + topic selections.
Imports Base from database.models (shared metadata) â€” same pattern as custom/challenge models.
"""
import uuid
...
from database.models import Base  # shared metadata â†’ one create_all


class UserOnboardingFlags(Base):
    """
    Stores the three onboarding flags per user.
    One row per user â€” created on first login.
    """
