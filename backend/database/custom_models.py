...
Imports Base from database.models (shared metadata) â€” same pattern as challenge_models.py.
...
class UserFactProgress(Base):
    ...
    id           = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id      = Column(UUID(as_uuid=True), ForeignKey("users.id"),        nullable=False)
    fact_id      = Column(UUID(as_uuid=True), ForeignKey("custom_facts.id"), nullable=False)
