...
if __name__ != "__main__":
    import pytest
    pytest.skip("Standalone integration script; run with python tests/e2e_test.py", allow_module_level=True)
...
# GET /status — correct prefix is /api/onboarding
...
if session_ids.get("classic") and question_ids.get("classic"):
    ...
    # doesn't persist the question (happens on cold start — not a code bug)
...
# Duplicate answer replay (idempotent)
if question_ids.get("challenge_l1") and session_ids.get("challenge"):
    ...
    # Implementation is idempotent: duplicate submits replay the stored answer
    test("Challenge: duplicate answer replayed (idempotent)", r, 200)
...
if session_ids.get("pvp_match"):
    # Progressive question answering: fetch match → answer current question → re-fetch
...
# Reset password with bad OTP — should be 400
...
if not bootstrap_ok:
    WARNINGS.append("Admin bootstrap failed — admin endpoint tests will be skipped or may fail with 403")
