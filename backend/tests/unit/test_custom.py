...
if __name__ != "__main__":
    import pytest
    pytest.skip("Script integration test; run with python test_custom.py", allow_module_level=True)
...
def bootstrap_auth_user():
    ...
    if status not in (200, 201):
        print(f"{RED}✗ Could not create custom test user: {status} {data}{RESET}")
        sys.exit(1)
    ...
    if status != 200:
        print(f"{RED}✗ Could not login custom test user: {status} {data}{RESET}")
        sys.exit(1)

    AUTH_TOKEN = data.get("access_token", "")
    TEST_USER_UUID = str((data.get("user") or {}).get("id", ""))
    if not AUTH_TOKEN or not TEST_USER_UUID:
        print(f"{RED}✗ Auth bootstrap returned incomplete payload: {data}{RESET}")
        sys.exit(1)

    print(f"  {YELLOW}ℹ Created auth-backed test user: {TEST_USER_UUID}{RESET}")


# ════════════════════════════════════════════════════════════════════════════
print("\n" + "═" * 60)
print("  AdaptIQ — Custom Room Backend Test Suite")
print("═" * 60 + "\n")
...
# ── 3. GET /api/custom/topics ─────────────────────────────────────────────────
status, data = get("/api/custom/topics", auth=True)
topics = data.get("topics", [])
check("GET /api/custom/topics — returns topic list",
...
check("POST /api/custom/start-session (History - World War II)",
      ...
      detail=f"session_id={str(data.get('session_id','?'))[:8]}…  progress={data.get('progress_percentage','?')}%",
      reason=f"Got {status}: {data}")
session_id_history = data.get("session_id", "")
progress_initial   = data.get("progress_percentage", -1)

check("Start session returns valid progress_percentage (0–100)",
...
check("POST /api/custom/start-session (Geography - France)",
      status == 200 and "session_id" in data,
      detail=f"session_id={str(data.get('session_id','?'))[:8]}…",
      reason=f"Got {status}: {data}")
session_id_geo = data.get("session_id", "")

# ── 6. Generate question — History ────────────────────────────────────────────
print(f"\n  {YELLOW}[LLM call — may take a few seconds…]{RESET}")
...
check("POST /api/custom/generate-question (History)",
      ...
      detail=f"Q: {str(data.get('text',''))[:60]}…",
      reason=f"Got {status}: {data}")
...
# ── 9. Submit WRONG answer ────────────────────────────────────────────────────
print(f"\n  {YELLOW}[LLM call — generating Q2 for wrong-answer test…]{RESET}")
...
if status == 200 and "id" in q2:
    ...
    fail("Submit wrong answer (skipped — Q2 generation failed)", f"{status}: {q2}")

# ── 10. Submit to non-existent question → 404 ────────────────────────────────
status, data = post("/api/custom/submit-answer", {
    "session_id":    session_id_history,
    "question_id":   "00000000-0000-0000-0000-000000000000",
    "answer":        "whatever",
}, auth=True)
check("Submit with bad question_id → 404", status == 404,
...
# ── 12. End session idempotent ────────────────────────────────────────────────
status, data = post(f"/api/custom/session/{session_id_history}/end", auth=True)
check("End session idempotent (2nd call → 200)", status == 200,
      detail=f"Got {status}", reason=f"Expected 200, got {status}: {data}")

# ── 13. End session bad ID → 404 ─────────────────────────────────────────────
status, data = post("/api/custom/session/00000000-0000-0000-0000-000000000000/end", auth=True)
check("End session bad session_id → 404", status == 404,
      detail=f"Got {status}", reason=f"Expected 404, got {status}: {data}")

# ── 14. Progress persists across sessions ─────────────────────────────────────
status, data = post("/api/custom/start-session", {
    "user_id": TEST_USER_UUID,
    "topic":   HISTORY_TOPIC,
}, auth=True)
check("Progress persists: new session returns updated percentage",
      ...
      reason=f"Expected ≥ {new_progress}%, got {data.get('progress_percentage')}%  (status {status})")
