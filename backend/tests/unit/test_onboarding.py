...
if __name__ != "__main__":
    import pytest
    pytest.skip("Script integration test; run with python test_onboarding.py", allow_module_level=True)
...
def bootstrap_auth_user(label: str):
    ...
    if status not in (200, 201):
        print(f"{RED}✗ Could not create {label} user: {status} {data}{RESET}")
        sys.exit(1)
    ...
    if status != 200:
        print(f"{RED}✗ Could not login {label} user: {status} {data}{RESET}")
        sys.exit(1)

    token = data.get("access_token", "")
    user_id = str((data.get("user") or {}).get("id", ""))
    if not token or not user_id:
        print(f"{RED}✗ Incomplete auth payload for {label}: {data}{RESET}")
        sys.exit(1)
...
print("\n" + "═"*60)
print("  AdaptIQ — Onboarding Backend Test Suite")
print("═"*60 + "\n")
...
print(f"  {YELLOW}ℹ Created onboarding test user: {TEST_USER_UUID}{RESET}")

# 2. Status — new user, needs onboarding
...
# 4. Status after survey — tour_needed=True
status, data = get(f"/api/onboarding/status?user_id={TEST_USER_UUID}", token=AUTH_TOKEN)
...
# 5. Survey idempotency — second call must return 409
status, data = post("/api/onboarding/survey", {
    "user_id":              TEST_USER_UUID,
    "topics_confident":     ["History - Ancient Rome"],
    "topics_want_to_learn": [],
}, token=AUTH_TOKEN)
check("Survey idempotent: 2nd call → 409",
