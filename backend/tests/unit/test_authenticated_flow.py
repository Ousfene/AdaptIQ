"""
Authenticated API Flow Smoke Test
Tests JWT token generation, user ownership checks, and protected endpoints
"""
import asyncio
import json
import uuid
...
if __name__ != "__main__":
    import pytest
    pytest.skip("Standalone smoke test script", allow_module_level=True)

BACKEND_URL = os.getenv("ADAPTIQ_BASE_URL", "http://localhost:8000")


async def test_authenticated_flow():
    """Test complete authenticated API flow against a running backend."""

    print("\n" + "=" * 70)
    print("Authenticated API Flow Smoke Test")
    print("=" * 70)
    ...
    try:
        async with httpx.AsyncClient(base_url=BACKEND_URL) as client:
            ...
            if response.status_code in [200, 400]:  # 400 might occur if no concepts exist
