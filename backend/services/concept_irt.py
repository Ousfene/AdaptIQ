...
class ConceptIRT:
    ...
    async def update_concept_theta(
        ...
        # IRT update: Î´Î¸ = Î± * (response - P(correct))
        p = irt_probability(theta_record.theta, beta)
        gradient = (1 if correct else 0) - p
        new_theta = theta_record.theta + ConceptIRT.LEARN_RATE * gradient

        # Clamp theta to valid range
        new_theta = max(
            ConceptIRT.THETA_RANGE[0],
            min(ConceptIRT.THETA_RANGE[1], new_theta),
        )

        # Variance decay: uncertainty decreases per response
        new_variance = theta_record.theta_variance * ConceptIRT.VARIANCE_DECAY_FACTOR

        # Determine mastery level
        mastery_level = ConceptIRT._theta_to_mastery(new_theta)
