...
def visual_question_needs_generation(visual_q: VisualQuestion, level: int) -> bool:
    ...
    if level == 5:
        ...
        bad_words = {"unknown", "other", "none of the above", "cannot determine", "cannot be determined", "all of the above", "insufficient data"}
        if bad_words.intersection(normalized):
            return True
...
async def get_visual_session(db: AsyncSession, session_id: str) -> Optional[VisualSession]:
    result = await db.execute(
        select(VisualSession).where(VisualSession.id == uuid.UUID(session_id))
    )
...
async def get_next_question(
    ...
    for low_try, high_try in ranges:
        ...
        globally_unseen = [q for q in candidates if str(q.id) not in global_seen]
        if globally_unseen:
            return globally_unseen[0]
    ...
    for low_try, high_try in ranges:
        ...
        session_unseen = [q for q in candidates if str(q.id) not in current_session_seen]
        if session_unseen:
            return session_unseen[0]
        elif candidates:
...
async def generate_and_store_question(
    ...
    try:
        ...
        if options_count == 0:
            ...
            if not wrong1 or wrong1.lower() in {"unknown", "other", "none of the above", "cannot determine", "cannot be determined", "all of the above", "insufficient data", ""}:
                # Query another random question of the same topic to get a real realistic distractor!
                stmt = select(VisualQuestion).where(
                    VisualQuestion.topic == topic,
                    VisualQuestion.id != visual_q.id,
                    VisualQuestion.correct_answer != None,
                    VisualQuestion.correct_answer != ""
                ).limit(5)
                res = await db.execute(stmt)
                db_qs = res.scalars().all()
                for dq in db_qs:
                    ans = dq.correct_answer.strip()
                    if ans.lower() != correct.lower() and ans.lower() not in {"unknown", "other", "none of the above", "cannot determine", "cannot be determined", "all of the above", "insufficient data", ""}:
                        wrong1 = ans
                        break
                if not wrong1:
                    wrong1 = f"Alternative {topic}"
            ...
            wrongs = [w for w in wrongs if w and w.lower() not in {"unknown", "other", "none of the above", "cannot determine", "cannot be determined", "all of the above", "insufficient data", ""}]
            
            if len(wrongs) < 3:
                # Query other random questions of the same topic to pad the distractors!
                stmt = select(VisualQuestion).where(
                    VisualQuestion.topic == topic,
                    VisualQuestion.id != visual_q.id,
                    VisualQuestion.correct_answer != None,
                ...
                for dq in db_qs:
                    ans = dq.correct_answer.strip()
                    if ans.lower() != correct.lower() and ans.lower() not in {w.lower() for w in wrongs}:
                        if ans.lower() not in {"unknown", "other", "none of the above", "cannot determine", "cannot be determined", "all of the above", "insufficient data", ""}:
                            wrongs.append(ans)
                            
            idx = 1
            while len(wrongs) < 3:
                wrongs.append(f"Option {idx}")
                idx += 1
                
            options = [correct] + wrongs[:3]
            seen_opt, unique = set(), []
            for o in options:
                if o.lower() not in seen_opt:
                    seen_opt.add(o.lower())
                    unique.append(o)
                    
            while len(unique) < 4:
                unique.append(f"Option {idx}")
                idx += 1
        ...
        fallback_correct = (visual_q.paragraph or "").strip()[:60] or f"Subject of {visual_q.topic}"
        ...
        if options_count == 0:
            ...
            wrong = ""
            stmt = select(VisualQuestion).where(
                VisualQuestion.topic == visual_q.topic,
                VisualQuestion.id != visual_q.id,
                VisualQuestion.correct_answer != None,
                VisualQuestion.correct_answer != ""
            ).limit(5)
            ...
            for dq in db_qs:
                ans = dq.correct_answer.strip()
                if ans.lower() != fallback_correct.lower() and ans.lower() not in {"unknown", "other", "none of the above", "cannot determine", "cannot be determined", "all of the above", "insufficient data", ""}:
                    wrong = ans
                    break
            if not wrong:
                wrong = f"Alternative {visual_q.topic}"
                
            options = [fallback_correct, wrong]
            random.shuffle(options)
            visual_q.options_json   = json.dumps(options)
            visual_q.question_type  = 'M'
        else:
            ...
            for dq in db_qs:
                ans = dq.correct_answer.strip()
                if ans.lower() != fallback_correct.lower() and ans.lower() not in {w.lower() for w in wrongs}:
                    if ans.lower() not in {"unknown", "other", "none of the above", "cannot determine", "cannot be determined", "all of the above", "insufficient data", ""}:
                        wrongs.append(ans)
            ...
            unique = [fallback_correct] + wrongs[:3]
            random.shuffle(unique)
            visual_q.options_json   = json.dumps(unique[:4])
            visual_q.question_type  = 'M'
