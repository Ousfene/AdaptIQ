"""
services/challenge_service.py â€” Business logic for the Challenge Room.
...
STREAK_UP_THRESHOLD   = 4   # 4 correct in a row â†’ level up
STREAK_DOWN_THRESHOLD = 2   # 2 wrong  in a row â†’ level down
...
# Which levels each rank can access â€” also defines the min/max level
...
def calculate_points(level: int, is_correct: bool) -> int:
    """Correct â†’ positive points. Wrong â†’ negative points."""
...
def check_streak_trigger(
    ...
    if streak_correct >= STREAK_UP_THRESHOLD:
        return {
            "direction": "up",
            "reason": f"Outstanding! {streak_correct} correct in a row â€” advancing to next level.",
        }
    if streak_wrong >= STREAK_DOWN_THRESHOLD:
        return {
            "direction": "down",
            "reason": f"{streak_wrong} wrong answers â€” dropping to a more suitable level.",
        }
    return None
...
def update_streaks_after_answer(
    ...
    """
    Correct â†’ increment correct streak, reset wrong streak.
    Wrong   â†’ increment wrong streak, reset correct streak.
...
async def update_global_ranking(
    ...
    if ranking.current_rank != old_rank:
        logger.info(
            f"User {user_id[:8]} promoted: {old_rank} â†’ {ranking.current_rank} "
