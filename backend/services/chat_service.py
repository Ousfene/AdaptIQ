...
async def retrieve_context(
    ...
    async def _fetch_wiki() -> Optional[str]:
        try:
            ...
            logger.warning("Wikipedia fetch timed out for chat question=%s", question[:50])
            return None
    ...
    async def _fetch_wikidata() -> Optional[str]:
        try:
            ...
            logger.warning("Wikidata fetch timed out for chat question=%s", question[:50])
            return None
        except Exception as exc:
            logger.warning(
...
async def synthesize_answer(
    ...
    if grounded and context:
        user_prompt = (
            f"QUESTION: {question}\n\n"
            f"TOPIC: {topic}\n\n"
            f"VERIFIED CONTEXT FROM SOURCES:\n{context[:2000]}\n\n"
            "Using the context above as your primary source, write a clear "
...
async def handle_ask(
    ...
    if not in_scope:
        logger.info(
            "Chat scope violation: user=%s question=%s",
            user_id[:8],
            question[:60],
        )
    ...
    if rag_pipeline is not None and http_client is not None:
        try:
            ...
            logger.warning(
                "RAG retrieval soft-timeout reached for chat: user=%s question=%s",
                user_id[:8],
                question[:50],
            )
