...
from config import CUSTOM_ROOM_SIMPLE_MODE
...
def _custom_simple_generation_payload(topic_label: str, fact_content: str) -> dict[str, Any]:
    return {
...
async def list_topics(current=Depends(get_current_user)):
    _ = current
...
async def start_session(body: StartSessionRequest, request: Request, current=Depends(get_current_user)):
    ...
    async for db in _get_db(request):
        ...
        total = total_in_db if total_in_db > 0 else total_from_catalogue
...
async def generate_question(body: GenerateQuestionRequest, request: Request, current=Depends(get_current_user)):
    ...
    async for db in _get_db(request):
        ...
        if CUSTOM_ROOM_SIMPLE_MODE:
            ...
            options = _normalize_generated_options(raw_options, correct_answer)
        ...
        target_difficulty = await _target_difficulty_for_user(
            db=db,
            user_id=session.user_id,
            topic_family=topic_family,
            selected_concept=effective_concept,
        )
        ...
        if CUSTOM_REQUIRE_RAG_GENERATION:
            ...
            for _ in range(3):
                ...
                question_source = f"custom_rag_{source_token}" if source_token else "custom_rag"
                break
            ...
            if not data:
                if llm_rate_limited:
                    raise HTTPException(
                        429,
                        "LLM rate limit reached. Please retry in a few seconds.",
                        headers={"Retry-After": "3"},
                    )
                raise HTTPException(502, "Unable to generate a RAG question right now. Please retry.")
        ...
        if repeat_row is not None:
            ...
            if repeat_q is not None:
                ...
                repeat_opts = _parse_options(repeat_q.options_json)
                if len(repeat_opts) >= 2:
        ...
        if db_question is not None:
            options = _parse_options(db_question.options_json)
            if len(options) >= 2:
        ...
        if not data:
            ...
            if emergency_q is not None:
                emergency_opts = _parse_options(emergency_q.options_json)
                if len(emergency_opts) >= 2:
...
async def generate_hint(
    ...
    async for db in _get_db(request):
        ...
        logger.debug(
            "custom.generate_hint - LLM inputs: question_text=%s correct_answer=%s body_provided=%s",
            resolved_question_text[:300],
            (resolved_correct_answer or "<empty>")[:120],
...
async def submit_answer(body: SubmitAnswerRequest, request: Request, current=Depends(get_current_user)):
    ...
    async for db in _get_db(request):
        ...
        total = total_in_db if total_in_db > 0 else 500
        mastery = await get_or_create_mastery(db, session.user_id, session.topic, total)

        if is_correct:
            mastery.mastered_facts_count = min(
                mastery.mastered_facts_count + 1,
                mastery.total_facts_count,
            )
        await _refresh_mastery_percentage(mastery)
