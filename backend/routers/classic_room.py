    ...
    try:
        ...
        if not question:
            ...
            stmt = select(QuestionBank).where(
                func.lower(QuestionBank.source).notin_(["challenge_llm", "custom_llm"])
            )
            ...
            if not fallback_q:
                any_stmt = (
                    select(QuestionBank)
                    .where(func.lower(QuestionBank.source).notin_(["challenge_llm", "custom_llm"]))
                )
                if governance_enabled:
...
async def generate_hint(
    ...
    try:
        ...
        logger.info("Hint generated", question_preview=body.question_text[:50])
...
async def submit_answer(
    ...
    try:
        ...
        return SubmitAnswerResponse(
            ...
            theta_updated=result.get("theta_change"),
