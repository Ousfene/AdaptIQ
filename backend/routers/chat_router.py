...
# ─── Dependency helpers ───────────────────────────────────────────────────────
# Follow the same pattern as routers/visual_room.py — read from app.state
# directly rather than using FastAPI Depends for app-level singletons.


async def _get_db(request: Request):
    """Yield a request-scoped AsyncSession from app.state."""
